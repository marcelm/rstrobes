    ...
    if mcs_strategy != McsStrategy::FirstStrobe {
        for randstrobe in query_randstrobes {
            if let Some(position) = index.get_full(randstrobe.hash) {
                ...
                let hit = Hit { position, query_start: randstrobe.start, query_end: randstrobe.end, is_partial: false, is_filtered: false, hash_revcomp: randstrobe.hash_revcomp };
                hits.push(hit);
            } else {
                ...
                if mcs_strategy == McsStrategy::Always {
                    if let Some(position) = index.get_partial(randstrobe.hash) {
                        ...
                        let hit = Hit { position, query_start: randstrobe.start, query_end: randstrobe.start + index.k(), is_partial: true, is_filtered: false, hash_revcomp: randstrobe.hash_revcomp };
                        hits.push(hit);
                    } else {
